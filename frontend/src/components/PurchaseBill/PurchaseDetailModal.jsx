...
const PurchaseBillDetailModal = ({ purchaseInvoiceId, onClose, onUpdate }) => {
    ...
    const fetchPurchaseBillDetails = async () => {
      try {
        ...
        const response = await axios.get(
          ...
          }/purchase-invoice/${purchaseInvoiceId}/detail`,
          {
            headers: {
  ...
  const fetchSupplierInfo = async (supplierId) => {
    try {
      const accessToken = window.localStorage.getItem("accessToken");
      const response = await axios.get(
        `${import.meta.env.VITE_LOCAL_API_URL}/supplier/${supplierId._id}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
      ...
      setSupplierInfo({
        ...
        supplierPhone: "Không có số điện thoại",
      });
    }
  };
  ...
  const handleApprove = async (status) => {
    try {
      ...
      if (response.status === 201 && response.data) {
        ...
        alert(
          `Hóa đơn đã được ${status === "approved" ? "phê duyệt" : "từ chối"
          } thành công`
        );
        onClose();
      } else {
        ...
        throw new Error(
          `Phê duyệt không thành công. Status: ${response.status
      ...
      alert(
        `Lỗi khi phê duyệt hóa đơn: ${error.response?.data?.message || error.message
        }`
      );
    }
  };
  ...
  const generatePDF = () => {
    ...
    doc.text("Chi tiết hóa đơn mua", 105, 15, null, null, "center");

    // Thông tin hóa đơn
    doc.setFontSize(12);
    doc.setFont("Roboto", "normal");
    doc.text(`Mã hóa đơn: ${purchaseBill.purchaseInvoiceId}`, 20, 30);
    doc.text(`Nhà cung cấp: ${purchaseBill.supplierId.supplierName}`, 20, 40);
    doc.text(`Số điện thoại: ${purchaseBill.supplierId.supplierPhone || "N/A"}`, 20, 50);
    doc.text(`Tổng tiền: ${purchaseBill.totalAmount?.toLocaleString() ?? "N/A"} đ`, 20, 60);

    // Bảng sản phẩm
    const tableColumn = ["Sản phẩm", "Số lượng", "Đơn giá", "Thành tiền"];
    const tableRows = purchaseBill.purchaseProducts?.map((product) => [
      product.productId?.productName,
      product.quantity,
      `${product.productId?.purchasePrice?.toLocaleString() ?? "N/A"} đ`,
      `${((product.quantity || 0) * (product.productId?.purchasePrice || 0)).toLocaleString()} đ`,
    ]);
    ...
    // Lưu PDF
    doc.save(`HoaDonMua_${purchaseBill.purchaseInvoiceId}.pdf`);
  };
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-semibold">
                {purchaseBill.supplierId.supplierName}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Số điện thoại</p>
              <p className="text-lg font-semibold">
                {purchaseBill.supplierId.supplierPhone || "012312312312"}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Tổng tiền</p>
              <p className="text-lg font-semibold text-green-600">
                {purchaseBill.totalAmount?.toLocaleString() ?? "N/A"} đ
              </p>
            </div>
            {purchaseBill.status !== "payed" && (
              <>
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {purchaseBill.paidAmount?.toLocaleString() ?? "N/A"} đ
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Số tiền còn nợ</p>
                  <p className="text-lg font-semibold">
                    {purchaseBill.amountOwed?.toLocaleString() ?? "N/A"} đ
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Thời hạn thanh toán</p>
                  <p className="text-lg font-semibold">
                    {purchaseBill.paymentTems} ngày
                  </p>
                </div>
              </>
            )}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {purchaseBill.purchaseProducts?.map((product, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {product.productId?.purchasePrice?.toLocaleString() ??
                          "N/A"}{" "}
                        đ
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {(
                          (product.quantity || 0) *
                          (product.productId?.purchasePrice || 0)
                        ).toLocaleString()}{" "}
                        đ
                      </td>
                    </tr>
          ...
          <div className="mt-8 flex justify-end space-x-4">
            ...
            <button
              type="button"
              onClick={generatePDF}
              className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-2 rounded-full hover:shadow-lg transition-all duration-300 ease-in-out font-semibold transform hover:scale-105"
            >
              In hóa đơn
            </button>
          </div>
