...
export class ProductsService {
  constructor(
    ...
    private readonly cloudinaryService: CloudinaryService, // Sử dụng dịch vụ Cloudinary
  ) {}
  ...
  async addProductService(data: addProductDto): Promise<Products> {
    ...
    const categoryExists = await this.categoryService.findCategoryByIdService(
      data.categoryId,
    );
    if (!categoryExists) {
    ...
    newProduct.productId = await this.generateService.generateId(
      this.productModel,
      'productId',
      'P',
    );
    try {
      ...
      if (error.code === 11000) {
        // Duplicate key error (trường hợp unique constraint bị vi phạm)
  ...
  // thêm ảnh sản phẩm
  // async addProductImageService(
  //   productId: string,
  //   productImage: string,
  // ): Promise<Products> {
  //   const product = await this.productModel.findById(productId).exec();
  //   const deleteImage = this.cloudinaryService.deleteImage(product.productImage);
  //   if (deleteImage) {
  //     return null;
  //   }
  //   const updatedProduct = await this.productModel.findOneAndUpdate({ productId }, { productImage }, { new: true }).exec();
  ...
  async deleteProductService(id: string): Promise<{ message: string }> {
    ...
    const warehouseWithProduct =
      await this.warehouseService.getAllWarehouseService();
    const isProductInWarehouse = warehouseWithProduct.some(
      (warehouse) => warehouse.productId.toString() === id,
    );
    if (isProductInWarehouse) {
      throw new BadRequestException(
        'Cannot delete product because it exists in a warehouse',
      );
    }
  ...
  async searchProductService(
    searchKey: string,
  ): Promise<{ message: string; products: Products[] }> {
  ...
  async filterProductByCategoryService(categoryId: string): Promise<any[]> {
    const query: any = {};
  ...
  // lọc sản phẩm theo danh mục dùng cho xóa danh mục
  async filterProductByCategoryForCategoryService(
    categoryId: string,
  ): Promise<any[]> {
    const query: any = {};
