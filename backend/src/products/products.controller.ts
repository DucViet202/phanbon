...
import { BadRequestException, Body, Controller, Delete, Get, InternalServerErrorException, NotFoundException, Param, Patch, Post, Put, Query, Req, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
import { ProductsService } from './products.service';
import { ApiBadRequestResponse, ApiBearerAuth, ApiBody, ApiConsumes, ApiCreatedResponse, ApiInternalServerErrorResponse, ApiNotFoundResponse, ApiOkResponse, ApiResponse, ApiTags } from '@nestjs/swagger';
...
export class ProductsController {
  ...
  // // @UseGuards(PermissionGuard) 
  // @UseInterceptors(FileInterceptor('image')) // Sử dụng FileInterceptor để xử lý file
  // async uploadImageController(
  //   @Param('id') id: string,
  //   @UploadedFile() file: Express.Multer.File,
  // ): Promise<{ message: string }> {
  //   const productId = id;
  //   const fileResult = await this.cloudinaryService.uploadImageService(productId.toString(),file);
  //   await this.productsService.updateProductService(productId, fileResult.uploadResult.url);
